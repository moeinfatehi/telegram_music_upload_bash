...
do
        ...
        title=$(echo "$fileInfo"|grep -ie ^title|cut -d":" -f2-10|cut -d" " -f2-10)
        album=$(echo "$fileInfo"|grep -ie ^album:|cut -d":" -f2-10|cut -d" " -f2-10)
        year=$(date -d $(echo "$fileInfo"|grep -i "recording date"|cut -d":" -f2-100|tr -d " ") '+%Y')
